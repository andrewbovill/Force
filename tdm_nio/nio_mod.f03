...
!
      ...
      subroutine fulltdm(TDM,Evals_A,Evals_B,dip_x,dip_y,dip_z,ddnos_alpha,ddnos_beta)
!
      implicit none
      integer(kind=real64)::i,j
      real(kind=real64),dimension(3)::TDM
      type(MQC_variable),intent(in)::Evals_A,Evals_B
      type(MQC_Variable)::dip_x,dip_y,dip_z,pddno,hddno,ddnos_alpha,ddnos_beta
      real(kind=real64),allocatable,dimension(:)::tmp_evals
      real(kind=real64)::tmp_val1
      
      write(*,*) "Subroutine Working"
      ...
      tmp_evals = Evals_A
      do i = 1,7  
      tmp_val1 = tmp_evals(i)
        if(abs(tmp_evals(i)).gt.0.999) then
        !if(abs(tmp_evals(i)).gt.1.d-5) then
          write(*,*) "I: ", i, "value", abs(tmp_evals(i))
          pDDNO = ddnos_alpha%column(i)
            do j = i, 7
              if(tmp_val1.eq.abs(tmp_evals(j))) then
                write(*,*) "pDDno", i
                write(*,*) "hDDno", j
                hDDNO = ddnos_alpha%column(j)
!               TDM(1) =  TDM(1) + tmp_val1*float(dot_product(pDDNO,MQC_Variable_MatrixVector(dip_x,hDDNO)))
!               TDM(2) =  TDM(2) + tmp_val1*float(dot_product(pDDNO,MQC_Variable_MatrixVector(dip_y,hDDNO)))
!               TDM(3) =  TDM(3) + tmp_val1*float(dot_product(pDDNO,MQC_Variable_MatrixVector(dip_z,hDDNO)))
                TDM(1) =  dot_product(pDDNO,MQC_Variable_MatrixVector(dip_x,hDDNO))
                TDM(2) =  dot_product(pDDNO,MQC_Variable_MatrixVector(dip_y,hDDNO))
                TDM(3) =  dot_product(pDDNO,MQC_Variable_MatrixVector(dip_z,hDDNO))
              end if
            end do
        endIf
      end do
      ...
      tmp_val1 = tmp_evals(i)
        ...
        !if(abs(tmp_evals(i)).gt.1.d-5) then
          write(*,*) "I: ", i, "value", abs(tmp_evals(i))
          pDDNO = ddnos_beta%column(i)
            do j = i, 7
              if(tmp_val1.eq.abs(tmp_evals(j))) then
                hDDNO = ddnos_beta%column(j)
!               TDM(1) =  TDM(1) + tmp_val1*float(dot_product(pDDNO,MQC_Variable_MatrixVector(dip_x,hDDNO)))
!               TDM(2) =  TDM(2) + tmp_val1*float(dot_product(pDDNO,MQC_Variable_MatrixVector(dip_y,hDDNO)))
!               TDM(3) =  TDM(3) + tmp_val1*float(dot_product(pDDNO,MQC_Variable_MatrixVector(dip_z,hDDNO)))
              end if
