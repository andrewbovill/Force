...
!
      ...
      type(MQC_Variable)::SMatrixAO,SMatrixEVecs,SMatrixEVals,  &
        SMatrixAOHalf,SMatrixAOMinusHalf,overlap
      ...
      type(MQC_Variable)::tmpMQCvar,tmpMQCvar1,tmpMQCvar2,tmpMQCvar3,  &
        tmpMQCvar4
      logical::doTestCode=.false.,doMatrixFileOut=.false.,isNIO,isDDNO,found=.false.
!
!     Format Statements
!
...
!
      call GMatrixFile1%getArray('ALPHA SCF DENSITY MATRIX',mqcVarOut=PMatrixAlpha1,foundOut=found)
...
!
      ...
      write(*,*) "Andrew printing out eigenvalues for alphas and betas"
...
!
      if(isDDNO) then
        ...
        endIf
        write(*,*) "ANdrew doing debugging here to see which parts match to one another"
        write(*,*) "IPlusOneAlpha", IPlusOneAlpha
        write(*,*) "IPlusOneBeta", IPlusOneBeta
        write(*,*) "IMinusOneAlpha", IMinusOneAlpha
        write(*,*) "IMinusOneBeta", IMinusOneBeta
      ...
      endIf
      call DDNOsAlpha%print(iOut,"Andrew wanting to see full DDNOsAlpha matrix")

      write(*,*) "andrew doing cool shit"
      write(*,*) "Andrew doing dipole between non hole/pair combinations "
      write(*,*)
      pDDNO = DDNOsAlpha%column(1)
      hDDNO = DDNOsAlpha%column(3)

      call pDDNO%print(header='Andrew printing out particle DDNO')
      call hDDNO%print(header='Andrew printing out hole DDNO')
      write(*,*) "Andrew doing dipole between non hole/pair combinations "
      ...
      transitionDipole(2) =  dot_product(pDDNO,MQC_Variable_MatrixVector(dipoleAOy,hDDNO))
      transitionDipole(3) =  dot_product(pDDNO,MQC_Variable_MatrixVector(dipoleAOz,hDDNO))
      call mqc_print(transitionDipole,6,header='Transition Dipole Moment andtrew modified')

      write(*,*) "Inverting dipole to see if it's the same...)"

      transitionDipole(1) =  dot_product(hDDNO,MQC_Variable_MatrixVector(dipoleAOx,pDDNO))
      transitionDipole(2) =  dot_product(hDDNO,MQC_Variable_MatrixVector(dipoleAOy,pDDNO))
      transitionDipole(3) =  dot_product(hDDNO,MQC_Variable_MatrixVector(dipoleAOz,pDDNO))
      call mqc_print(transitionDipole,6,header='Transition Dipole Moment andtrew modified')
!
!     Andrew calculate TDM different way...
!
      ...
      call fulltdm(transitionDipole,diffDensityAlphaEVals,diffDensityBetaEVals, &
        dipoleAOx,dipoleAOy,dipoleAOz,DDNOsAlpha,DDNOsBeta)
      write(*,*) "AFter my new procedure"

      call mqc_print(transitionDipole,6,header='NEW Transition Dipole Moment andtrew modified')
